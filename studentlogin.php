			...
			if($num==0){
				...
				if(($timedif/60)<15){
					...
					echo 'alert("Attendance taken!!");
						window.location.href="http://localhost/studentafterlogin.php?usn='.$name.'&batch='.$batch.'&sem='.$i_info[3].'&lab='.$lab.'&subject='.$i_info2[4].'"';
					echo '</script>';
					mysqli_close($con);
				...
				echo '<script language="javascript">';
				echo 'window.location.href="http://localhost/studentafterlogin.php?usn='.$name.'&batch='.$batch.'&sem='.$i_info[3].'&lab='.$lab.'&subject='.$i_info2[4].'"';
				echo '</script>';
			}

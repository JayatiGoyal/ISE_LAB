...
<body>
    ...
    <script src="like_button.js"></script></li>
			</ul>
		</div>
...
if(($sem==4 && $sub!="USP")&&($sem==4 && $sub!="DAA"))
	{
		echo '<script language="javascript">';
		echo 'alert("Sunject doesn\'t exist for the chosen semester!");
		window.location.href="http://localhost/lab/adminstatus.html"';
...
if(($sem==6 && $sub!="DBMS")&&($sem==6 && $sub!="SET"))
	{
		echo '<script language="javascript">';
		echo 'alert("Sunject doesn\'t exist for the chosen semester!");
		window.location.href="http://localhost/lab/adminstatus.html"';
...
$q2="select distinct Date from attend  WHERE sem='$sem' AND subject='$sub' AND batch='$batch' order by Date asc";
$res2=mysqli_query($con,$q2);

/*
while( $row = mysqli_fetch_array($res2)){
    $days = date('l',strtotime($row['Date'])); // Inside while loop
}
